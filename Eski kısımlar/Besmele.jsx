...
export default function Besmele({ theme, sureId, sureNo, ayetSayisi, player }) {
  ...
  // Sadece Besmele çalınıyorsa true olacak
  const caliniyor =
    player?.durum === "caliyor" &&
    player?.aktifAyet?.besmeleIcin === sureNo

  const KucukRozet = ({ x, y }) => {
    // Çiçeğin tüm içeriğini bir group içinde toplayalım
    const cicekIcerik = (
      <>
        {[...Array(12)].map((_, i) => (
          <circle
    ...
    <g transform={`translate(${x}, ${y})`}>
      <svg
        ...
        overflow="visible"
        style={{
          animation: caliniyor ? `rozetSpin_${x}_${y} 12s linear infinite` : "none",
        }}
      >
        <style>{`
          @keyframes rozetSpin_${x}_${y} {
      ...
      <div style={{
        ...
        >
          ...
          <text
            ...
            fontWeight="500"
            fill={ac} 
            fillOpacity={caliniyor ? "0.95" : "0.7"}
            direction="rtl"
          ...
          {/* Besmele'nin alt çizgisi - sadece Besmele okunurken görünür */}
          <rect
            x={w/2 - (caliniyor ? 130 : 0)}
            y={h/2 + 13}
            width={caliniyor ? 260 : 0}
            height="1.4"
            rx="0.7"
            fill={ac}
            fillOpacity={caliniyor ? "0.55" : "0"}
            style={{ transition: "width 0.4s ease, x 0.4s ease, fill-opacity 0.4s ease" }}
          />
        </svg> 
      </div>
    </div>
