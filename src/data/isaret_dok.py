...
ölçmek. Unicode adına da tek bir âyetteki görüntüye de güvenmiyoruz; sayıyoruz.
Bu yaklaşım şimdiye kadar iki hata yakaladı:
  • U+08D5 "kıraat farkı" sanılıyordu → 156 kez geçiyor, yani ص vakf-ı murahhas.
  • U+06DC "bu veride ط durağı" sanılıyordu → tam 4 kez ve tam Hafs'ın sekte
    yerlerinde geçiyor; asıl karışıklık fontta (06DC ile 0615 aynı glife düşüyor).

Kullanım:
    python3 isaret_dok.py <dosya>

Ek kipler:
    --kod 06E1        tek bir kodun TÜM geçtiği yerler
    --ayet 2:41       bir âyetin karakter karakter dökümü
    --ara قليلا       işaretleri atarak kelime arama
    --imla            İMLÂ DENETİMİ: farklı yazım üslubuyla gelmiş kelimeleri
                      bulur ve aynı âyetteki komşularıyla karşılaştırır
...
# DİKKAT — bu aralıklar Unicode'un BİRLEŞEN (Mn) kategorisinden üretildi.
# Önceki hâli 0x08D3'ten başlıyordu ve U+08D1/U+08D2'yi GÖREMİYORDU; bu yüzden
# "veride hiç geçmiyor" diye yanlış rapor edilip uygulamadan kaldırılmışlardı.
# Oysa U+08D1 (LARGE CIRCLE BELOW = zâid/okunmayan harf) veride geçiyor:
# "اُو۟لٰٓئِكَ" kelimesindeki okunmayan vâv'ı işaretliyor. Aralığı daraltma.
ARALIK = [
    (0x0610, 0x061A), (0x064B, 0x065F), (0x0670, 0x0670),
    (0x06D6, 0x06ED), (0x0898, 0x089F), (0x08CA, 0x08E1), (0x08E3, 0x08FF),
    (0xFBB2, 0xFBC1),
]
...
def main():
    ...
    UYGULAMA = {  # MushafKelime/KuranOkuma'nın BUGÜN tanıdığı kodlar
        0x0615, 0x0617, 0x06D6, 0x06D8, 0x06D9, 0x06DA, 0x06DB, 0x06DC,
        0x06E3, 0x06EA, 0x06EB, 0x06EC, 0x06ED,
        0x08D1, 0x08D5, 0x08D6, 0x08D7, 0x08D9, 0x08DE,
    }
    ...
    # ── 5a) İMLÂ DENETİMİ ───────────────────────────────────────────────────
    # Mushafın çoğu bir imlâ üslubuyla yazılmış; küçük bir bölüm başka üslupla
    # gelmiş olabilir (ör. U+0671 ٱ ve U+06E1 ۡ kullanan Osmanî yazım). Karışık
    # imlâ aynı sayfada göze çarpar ve arama/normalize işlerini de bozar.
    if imla:
        # Üslup göstergeleri: solda "yaygın", sağda "öteki"
        CIFTLER = [
            (0x0652, 0x06E1, "sükûn"),
            (0x0627, 0x0671, "elif / vasl elifi"),
            (0x0656, 0x06EA, "uzun î"),
        ]
        OTEKI = {c[1] for c in CIFTLER}
        kelimeler = [(etiket(k), m) for m, k, a2 in kayitlar if alan_adi(a2) == "arabic"]
        print("\n" + "=" * 74)
        print("5) İMLÂ DENETİMİ")
        print("=" * 74)
        # DİKKAT: elif (U+0627) ve vasl elifi (U+0671) HARFTİR, işaret sayacında
        # yer almazlar. Bu yüzden üslup sayımı metinden doğrudan yapılır.
        harf_sayac = Counter()
        for metin, konum, a2 in kayitlar:
            if alan_adi(a2) != "arabic":
                continue
            for ch in metin:
                harf_sayac[ord(ch)] += 1
        print("Üslup göstergeleri (yaygın / öteki) — yalnız `arabic` alanında:")
        for yay, ot, adi in CIFTLER:
            print(f"  {adi:<18} U+{yay:04X} {harf_sayac.get(yay,0):>7} kez   ↔   "
                  f"U+{ot:04X} {harf_sayac.get(ot,0):>5} kez")

        şüpheli = [(kid, kel) for kid, kel in kelimeler
                   if any(ord(c) in OTEKI for c in kel)]
        print(f"\nÖteki üslupla yazılmış KELİME sayısı: {len(şüpheli)}")
        if not şüpheli:
            print("  (karışık imlâ yok)")
        else:
            # Etkilenen âyetleri topla: "2:105:24" -> "2:105"
            ayetler = {}
            for kid, kel in şüpheli:
                p2 = kid.split(":")
                ayetler.setdefault(":".join(p2[:2]), []).append(kid)
            print(f"Etkilenen ÂYET sayısı: {len(ayetler)}\n")
            for ay in sorted(ayetler, key=lambda x: [int(t) for t in x.split(":") if t.isdigit()]):
                print(f"── {ay}  ({len(ayetler[ay])} kelime)")
                for kid, kel in kelimeler:
                    if ":".join(kid.split(":")[:2]) != ay:
                        continue
                    im = "".join(f" U+{ord(c):04X}" for c in kel if ord(c) in OTEKI)
                    isaret = ">>" if im else "  "
                    print(f"   {isaret} {kid:<12} {kel}{'   ← ' + im.strip() if im else ''}")
                print()
