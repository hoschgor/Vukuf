...
let mushafBellek = null

/* Mushaf verisi 5 MB — yalnız ses bölümü AÇILDIĞINDA çekiliyor ve modülde
   tutuluyor. Servis işçisi zaten önbelleğe aldığı için ikinci çağrı anında. */
export async function mushafYukle() {
  if (mushafBellek) return mushafBellek
  const y = await fetch("/kuran-mushaf.json")
  if (!y.ok) throw new Error("Mushaf verisi alınamadı")
  mushafBellek = await y.json()
  return mushafBellek
}
