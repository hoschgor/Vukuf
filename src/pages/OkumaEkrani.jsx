...
// Lügat vurgusunun zemini: lügat renginin %15'i.
// NEDEN ELLE ÇÖZÜLÜYOR: `${renk}26` kısayolu YALNIZ 6 haneli hex'te doğru çalışır.
// Kullanıcı renk seçiciden hep #rrggbb veriyor ama temadan 3 haneli (#a07) bir renk
// gelirse `#a0726` 5 haneli geçersiz bir değer olur ve zemin sessizce KAYBOLUR —
// mushaf tarafında tam olarak bu tuzağa düşülmüştü. Burada 3 hane genişletiliyor,
// tanınmayan biçimlerde (rgb(), renk adı, color-mix) nötr bir griye düşülüyor.
function lugatZemin(renk) {
  const r = String(renk || "").trim()
  const k3 = /^#([0-9a-f])([0-9a-f])([0-9a-f])$/i.exec(r)
  if (k3) return `#${k3[1]}${k3[1]}${k3[2]}${k3[2]}${k3[3]}${k3[3]}26`
  if (/^#[0-9a-f]{6}$/i.test(r)) return `${r}26`
  if (/^#[0-9a-f]{8}$/i.test(r)) return r            // zaten alfalı
...
function MetinParcasi({
  ...
  // ZORUNLU ORTALAMA — yalnız Risale eserlerinde. Risale metinlerinde ⟦C⟧ ön eki künye,
  // imza ve ayraç satırları için konmuştu; oradaki ortalama metnin bir parçası. Risale
  // HARİCİ eserlerde ise bu, çerçeve oturana kadar konmuş bir iskeleydi ve kullanıcının
  // hizalama ayarını hükümsüz bırakıyordu (ayar değişiyor, metin ortada kalıyordu).
  // false verilince hem ⟦C⟧ hem de 1. sayfa kuralı yok sayılır; ⟦C⟧ işareti YİNE de
  // metinden ayıklanır (ayıklama ortalamaya bağlanırsa ekranda "⟦C⟧" yazısı görünürdü).
  zorunluOrtalama = true,
  // Açık lügat baloncuğunun anahtarı. Öbeğin BÜTÜN kelimeleri aynı anahtarı taşır
  // (lgKey `...-c-<grup>`), o yüzden vurgu tek kelimeye değil öbeğin tamamına iner.
  aktifLugatKey = null, onLugatSecim,
}) {
  const [secimBaslangic, setSecimBaslangic] = useState(null)
  // VURGU ARTIK ZAMANLAYICIYLA DEĞİL, BALONCUĞUN ÖMRÜYLE YAŞIYOR.
  // Eskiden tıklayınca 1.4 sn yanıp sönen yerel bir `basili` durumu vardı; ayrıca
  // kelimelerde inline `onMouseEnter/onMouseLeave` ile opacity 0.75 uygulanıyordu.
  // iOS'ta dokunma sahte bir `mouseenter` üretiyor ama `mouseleave` başka bir yere
  // dokunulana kadar GELMİYOR — kullanıcının "iOS'ta tıkladığım kelimede kalan efekt"
  // dediği şey buydu: yalnız parmağın değdiği kelimeye inen, öbeğin diğer kelimesine
  // geçmeyen bir kalıntı. Artık vurgu bizim kendi durumumuz: baloncuk açıkken öbeğin
  // tamamında durur, baloncuk kapanınca gider, her cihazda aynıdır. Fare üzerinde
  // gezinme efekti ise CSS'e taşındı (@media (hover: hover)) — dokunmatikte hiç çalışmaz.
  const vurguluMu = (key) => !!key && aktifLugatKey === key
  const basiliYak = (key) => onLugatSecim?.(key)
  // Efekt: lügat renginde ALT ÇİZGİ (eskiden olduğu gibi) + hafif zemin.
  // skipInk:none → g/y gibi inen harflerde çizgi kesilmez.
  // Zemin, satır sonunda bölünen kelimede de düzgün dursun diye box-decoration-break:clone.
  // Dolgu/kenarlık YOK: sayfa akışını kaydırmasın (vurgu açılıp kapanırken metin oynamasın).
  const basiliZemin = (key) => (vurguluMu(key)
    ? {
        textDecoration: "underline", textDecorationColor: lugatRenk,
        textUnderlineOffset: "0.18em", textDecorationThickness: "1.5px",
        textDecorationSkipInk: "none", WebkitTextDecorationSkipInk: "none",
        background: lugatZemin(lugatRenk), borderRadius: "3px",
  ...
  return (
    <div style={{ fontSize: `${fontSize}px`, fontFamily: metinFont, overflowWrap: "anywhere", wordBreak: "break-word", maxWidth: "100%" }}>
      {satirlar.map((satir, si) => {
        ...
        // ⟦C⟧ ön eki: ortalı normal satır (künye, imza, ayraç). İŞARET HER ZAMAN AYIKLANIR;
        // ortalanıp ortalanmayacağına `zorunluOrtalama` karar verir (bkz. prop notu).
        const merkezIsareti = satir.startsWith("⟦C⟧")
        const gosterilecek = merkezIsareti ? satir.replace(/^⟦C⟧/, "") : satir
        const merkez = merkezIsareti && zorunluOrtalama
...
// Aa panelinde önizlemenin ALTINDA ve "YAZI TİPİ" düğmesinde yazan ad — tek yerden.
// İki yerde iki ayrı ifade yazılsaydı biri değiştiğinde diğeri sessizce eskirdi.
const aktifMetinFontAdi = fontBul(fontSecimler.turkce || fontSecimler.osmanlica || "bookerly").label || "Varsayılan"
...
// Risale eseri mi? Tek ölçüt yazarın Nursî olması — başlık fontu zaten buna bakıyordu,
// zorunlu ortalama da aynı ölçüte bağlandı ki iki yerde iki ayrı tanım oluşmasın.
const risaleEseri = /Nurs[iî]/.test(kitap?.yazar || "")
const baslikFont = risaleEseri ? "LivaNur, serif" : metinFont
...
// TEK AÇIK BÖLÜM. Panel çok uzamıştı — altı kaydırıcı tek başına ekranı
...
// VARSAYILAN "sayfa": yalnız bulunulan sayfa yazar, toplam yazmaz. Eskiden "tam"dı
// (12 / 486); toplam sayı barda yer kaplıyor ve okurken işe yaramıyordu — mushaf
// tarafının varsayılanı da "sayfa". Kullanıcının kayıtlı tercihi varsa o korunur.
const [sayfaGosterim, setSayfaGosterim] = useState(() => localStorage.getItem("vukuf-sayfa-gosterim") || "sayfa")
...
// Açık baloncuğun ait olduğu lügat kalıbının anahtarı → o kalıbın BÜTÜN kelimeleri
// (öbekse ikisi de) baloncuk açık kaldığı sürece vurgulu durur. Baloncuk nerede
// kapatılırsa kapatılsın (çarpı, dışarı tıklama, sayfa değişimi, Esc) tek bir yerden
// temizlensin diye `popup`a bağlı bir etki kullanılıyor; her setPopup(null) çağrısına
// ayrı ayrı temizleme eklemek, bir tanesini atlayınca vurgunun ekranda unutulmasına
// yol açardı.
const [aktifLugatKey, setAktifLugatKey] = useState(null)
useEffect(() => { if (!popup) setAktifLugatKey(null) }, [popup])
...
const AaPanel = aaAcik && (
  <>
    ...
    {/* PANELİN ÜST DOLGUSU YOK (padding: "0 16px 16px").
        Üstteki blok `position: sticky`. Eskiden panelin dolgusunu negatif marjla
        telafi etmeye çalışıyordu ama YANLIŞ SAYIYLA: buradaki panelin dolgusu
        16px, blok ise 12px'e göre yazılmıştı (mushaf panelinden kopyalanmıştı).
    ...
    <div ref={aaPanelRef} className="okuma-panel" style={{ ...panelStil("center"), padding: "0 16px 16px", width: "300px", maxHeight: "80vh", overflowY: "auto" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: "11px", color: theme.textSecondary, marginBottom: "12px", letterSpacing: "1px", textAlign: "center" }}>
          YAZI TERCİHLERİ
        </div>
        <div style={{
      ...
      {/* METİN HİZALAMA — AÇILIR BÖLÜME KONMADI, ÜSTELİK TEK DOKUNUŞ.
          ...
          Üç seçenek yazı yerine İKON: "Sola / İki Tarafa / Orta" satırı kaplıyor,
      ...
      <div style={{
        ...
        <span style={{ flex: 1, minWidth: 0, fontSize: "10px", letterSpacing: "1px", color: theme.textSecondary }}>
          METİN HİZALAMA
        </span>
        <span style={{ display: "flex", gap: "4px", flexShrink: 0 }}>
          {[
            { value: "left", label: "Sola", Ikon: AlignLeft },
            { value: "justify", label: "İki tarafa", Ikon: AlignJustify },
            { value: "center", label: "Ortaya", Ikon: AlignCenter },
          ].map(h => (
            <button
              key={h.value}
              ...
              title={h.label}
              aria-label={h.label}
              aria-pressed={hizalama === h.value}
              style={{
                display: "flex", alignItems: "center", justifyContent: "center",
                width: "30px", height: "26px", borderRadius: "7px", cursor: "pointer",
                background: hizalama === h.value ? theme.accent : `${theme.accent}12`,
                color: hizalama === h.value ? "#fff" : theme.textSecondary,
                border: "none",
              }}
            >
              <h.Ikon size={14} />
...
.lugat-kelime, .lugat-kelime * { -webkit-tap-highlight-color: transparent; }
/* Fare üzerinde gezinme efekti. INLINE onMouseEnter/onMouseLeave YERİNE CSS:
   iOS dokunuşta sahte bir mouseenter üretip mouseleave'i göndermediği için inline
   sürüm, dokunulan kelimede kalıcı bir soluklaşma bırakıyordu. (hover: hover) sorgusu
   bunu gerçek fareli cihazlara kilitler; dokunmatikte hiç çalışmaz. */
@media (hover: hover) and (pointer: fine) {
  .lugat-kelime:hover { opacity: 0.75; }
}`}</style>
              ...
              <SayfaBlok
                ...
                render={(mod) => (
                  <MetinParcasi
                    ...
                    aktifLugatKey={aktifLugatKey}
                    onLugatSecim={setAktifLugatKey}
                    theme={theme}
                    fontSize={yaziBoyutu}
                    ...
                    ortala={risaleEseri && sayfa.sayfa === 1 && (sayfa.metin || "").split("\n").filter(l => l.trim() && !l.startsWith("§")).length <= 6}
                    zorunluOrtalama={risaleEseri}
                    lugatRenk={lugatRenk}
                    arapRenk={arapRenk}
